...
must_haves:
  truths:
    ...
    - "History is limited to 100 entries to prevent unbounded growth"
    ...
    **3. Create `src/calculator/ui/history_panel.py`** -- Scrollable history panel:
    ...
    class HistoryPanel(ctk.CTkFrame):
        ...
        def add_entry(self, expression, result):
            ...
            entry_frame.pack(fill="x", pady=2, before=self.scrollable_frame.winfo_children()[0] if self.entry_widgets else None)

            # Actually, pack ordering: we want newest at top.
            # Re-pack: destroy and re-pack all, or use pack with before/after.
            # Simplest approach: pack at the beginning by using side="top" and
            # repacking. But CTk frames don't support insert easily.
            # Better: just pack normally (newest at bottom) and scroll to bottom.
            # OR: pack with before= first existing widget.
            #
            # Correction: Use pack(fill="x", pady=2) and the frame will appear
            # at the bottom. For newest-first, we need to pack before existing widgets.
            # Actually, let's just pack normally (oldest at top, newest at bottom)
            # which is the natural reading order for a history/log.
    ...
    IMPORTANT simplification for the add_entry method: Remove the complex "insert at top" logic. Just use `entry_frame.pack(fill="x", pady=2)` which appends at the bottom. This gives chronological order (oldest at top, newest at bottom) which is natural for a log. The scrollable frame will auto-scroll or the user can scroll to see latest entries.

    Make sure the `add_entry` method is clean -- remove the commented-out deliberation block shown above. The final version should just do:
    ```python
...
<task type="auto">
  ...
  <action>
    ...
    **2. Modify `src/calculator/controller/calculator_controller.py`** -- Add history state management.
    ...
    Note: The clear_history on the controller is available but the HistoryPanel's clear button calls its own clear_history directly (it only clears the UI). To keep data and UI in sync, we need the panel's clear button to go through the controller.

    Solution: In __init__, DON'T let the panel's button call panel.clear_history directly. Instead, override by setting a callback. Modify HistoryPanel to accept an external clear callback:

    Actually, simpler: The HistoryPanel.clear_history() method already handles the UI. The controller just needs to also clear its data list. Two approaches:

    a) Have the panel's clear button call a callback that goes to the controller
    b) Have the controller periodically sync (bad)

    Go with (a): In HistoryPanel.__init__, change the clear_button command to call `self._on_clear`:
    ```python
    ...
    In CalculatorController.__init__, add:
    ```python
    ...
    def _on_history_cleared(self):
        """Called when user clicks clear history button in UI."""
        self.history.clear()
    ```

    This way the clear button in the panel: (1) clears UI widgets via panel.clear_history(), (2) notifies controller via callback to clear data list.
